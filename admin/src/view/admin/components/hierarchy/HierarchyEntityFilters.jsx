...
import {
  HierarchyStatuses,
  getHierarchyStatusOptionByValue,
...
const FILTER_CASCADE_RESETS = {
  communityId: ["vanshId", "kulId", "khampId", "subKhampId"],
  vanshId: ["kulId", "khampId", "subKhampId"],
  kulId: ["khampId", "subKhampId"],
  khampId: ["subKhampId"],
};

const HierarchyEntityFilters = ({
  filterFields,
  values,
  onChange,
  onParentChange,
  ...
  React.useEffect(() => {
    if (!filterFields.some((field) => field.fetchKey === "communities")) {
      return;
    }
  ...
  const loadStatusOptions = React.useCallback(() => HierarchyStatuses, []);
    ...
    <Card className="common-panel-card mb-3">
      <Card.Body>
        <Row className="g-3 align-items-start">
          <Col md={6} lg={4}>
            <Form.Group controlId="hierarchyFilterName">
          ...
          <Col md={6} lg={4}>
            <Form.Group controlId="hierarchyFilterStatus">
              <Form.Label>Status</Form.Label>
              <CustomSelect
                className="entity-form__select"
                value={getHierarchyStatusOptionByValue(values.status)}
                onChange={(option) =>
                  onChange({
          ...
          {filterFields.map((field) => {
            ...
            return (
              <Col md={6} lg={4} key={field.field}>
                <Form.Group controlId={`hierarchyFilter-${field.field}`}>
...
HierarchyEntityFilters.propTypes = {
...
export { FILTER_CASCADE_RESETS };
export default HierarchyEntityFilters;
