...
/**
 * Update membership-related settings (auth, payment gateway, referral)
 */
export const updateMembershipSettings = (formData) => async (dispatch) => {
  dispatch(removeErrors());
  dispatch(loadingOnCommonSettingsSubmit());
  dispatch(removeAlert());
  try {
    ...
    if (res.data.status === true) {
      dispatch(commonSettingsUpdated(res.data.response));
      dispatch(
        setAlert("Membership settings updated successfully.", "success"),
      );
    } else {
      const errors = res.data.errors;
      if (errors) {
        ...
        dispatch(setAlert("Failed to update membership settings.", "danger"));
      }
    }
  } catch (err) {
    if (err.response?.data?.tokenStatus === 0) {
      ...
      err.response &&
        dispatch(
          commonSettingsError({
            msg: err.response.statusText || "Error updating membership settings",
            status: err.response.status || 500,
          }),
        );
      ...
      dispatch(
        setAlert(
          err.response?.data?.message || "Failed to update membership settings",
          "danger",
        ),
      );
    }
