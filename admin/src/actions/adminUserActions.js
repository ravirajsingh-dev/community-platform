...
export const getUserReferrals =
  ...
  async (dispatch) => {
    try {
      const res = await api.get(`/api/admin/users/${userId}/referrals`, {
        params,
      ...
      if (err.response?.data?.tokenStatus === 0) {
        ...
        dispatch(
          setAlert(
            err.response?.data?.message || "Failed to fetch referred users",
            "danger",
          ),
        );
      }
...
const membershipActionRequest =
  (userId, action, payload, successFallback) => async (dispatch) => {
    dispatch(removeErrors());
    try {
      const res = await api.post(
        `/api/admin/users/${userId}/membership/${action}`,
        payload,
      );
      ...
      dispatch(
        setAlert(
          res.data?.message || `Failed to ${action} membership`,
          "danger",
        ),
      );
...
export const activateUserMembership = (userId, payload) =>
...
export const renewUserMembership = (userId, payload) =>
  membershipActionRequest(
    userId,
    "renew",
    payload,
...
export const blockUserMembership = (userId, payload) =>
  membershipActionRequest(
    userId,
    "block",
    payload,
...
export const unblockUserMembership = (userId, payload) =>
  membershipActionRequest(
    userId,
    "unblock",
    payload,
...
export const expireUserMembership = (userId, payload) =>
  membershipActionRequest(
    userId,
    "expire",
    payload,
