...
module.exports.adminLogin = async (req, res) => {
  ...
  try {
    ...
    const bruteForceCheck = checkBruteForceProtection(admin_id);
    ...
    delete sanitizedUser.admPwdRef;
    delete sanitizedUser.txnRef;
    delete sanitizedUser.passCopy; // Virtual - safe to delete even if not present
    delete sanitizedUser.txnPassCopy; // Virtual - safe to delete even if not present
  ...
  try {
    ...
    if (userRole === 2) {
      admin = await Admin.findById(userId)
        .select("-password -admPwdRef -txnRef -passCopy -txnPassCopy")
        .lean();
    } else if (userRole === 3) {
    ...
    if (userRole === 2) {
      updatedUser = await Admin.findByIdAndUpdate(
        { _id: userId },
        {
          ...
          admPwdRef: newPassword, // Store plain text copy for admin view (will be encrypted by pre-update hook)
  ...
  try {
    ...
    let updatedUser = await Admin.findByIdAndUpdate(
      { _id: user._id },
      {
        ...
        txnRef: txn_password, // Store plain text copy for admin view (will be encrypted by pre-update hook)
      },
      { returnDocument: "after" },
    ).lean();
    ...
    if (userRole === 2) {
      updatedUser = await Admin.findByIdAndUpdate(
        { _id: userId },
        {
          ...
          txnRef: newTxnPassword, // Store plain text copy for admin view (will be encrypted by pre-update hook)
        },
        { returnDocument: "after" },
      ).lean();
    } else {
